...
function Layout:Reload(unit)
	...
	for frame in pairs(ShadowUF.Units.frameList) do
		if( ( not unit or frame.unitType == unit ) and not frame.isHeaderFrame ) then
			frame:SetVisibility()
			self:Load(frame)
			frame:FullUpdate()
		end
...
function Layout:AnchorFrame(parent, frame, config)
	...
	local scale = 1
	if( config.anchorTo == "UIParent" and frame.unitType and GetCVar("useUiScale") ) then
		scale = frame:GetScale() * GetCVar("uiScale")
	end
