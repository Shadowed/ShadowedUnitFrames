local Indicators = {}
local indicatorList = {"status", "pvp", "leader", "masterLoot", "raidTarget", "happiness", "ready", "role"}
local leavingWorld
...
function Indicators:OnDisable(frame)
	...
	for _, key in pairs(indicatorList) do
...
function Indicators:TestMode(frame)
	...
	if( frame.indicators.raidTarget and frame.indicators.raidTarget.enabled ) then
		if( not frame.indicators.raidTarget.wasSet ) then
	...
	if( frame.indicators.role and frame.indicators.role.enabled ) then
		if( not frame.indicators.role.wasSet ) then
	...
	if( frame.indicators.pvp and frame.indicators.pvp.enabled ) then
		if( not frame.indicators.pvp.wasSet ) then
	...
	if( frame.indicators.ready and frame.indicators.ready.enabled ) then
		if( not frame.indicators.ready.wasSet ) then
			local chance = math.random(1, 3)
