...
local unitFrames, frameList, loadedUnits, unitEvents, queuedCombat = Units.unitFrames, Units.frameList, Units.loadedUnits, Units.unitEvents, {}
...
function Units:CheckGroupedUnitStatus(frame)
	...
	-- be non-existance while the primary unit is. So if we see they're in a vehicle with this case, we force the full update to get the vehicle change
...
local function createChildUnits(self)
	if( self.unitType == "party" and self.unitRealType == "party" ) then
		if( loadedUnits.partypet ) then
			Units:LoadChildUnit(self, "partypet", self.unitID)
		end
		
		if( loadedUnits.partytarget ) then
			Units:LoadChildUnit(self, "partytarget", self.unitID)
		end
	elseif( ( self.unitType == "maintank" or self.unitType == "mainassist" ) and loadedUnits[self.unitType .. "target"] ) then
		Units:LoadChildUnit(self, self.unitType .. "target", self.unitID)
	end
end
...
local function OnAttributeChanged(self, name, unit)
	...
	if( not self.unitType or ( self.unitRealType == self.unitType ) ) then
	...
	-- Create child frames
	if( self.unitType == "maintank" or self.unitType == "mainassist" or self.unitType == "party" ) then
		createChildUnits(self)
	end
	...
	elseif( string.match(self.unit, "%w+target") ) then
		...
		elseif( self.unitType == "maintanktarget" or self.unitType == "mainassisttarget" ) then
	...
	-- Mainain a simple list of active frames for when we need to do full updates of everything
...
function Units:LoadGroupHeader(config, type)
	...
	if( type == "party" ) then
		...
		stateMonitor:WrapScript(stateMonitor, "OnAttributeChanged", [[
			...
			if( self:GetAttribute("hideAnyRaid") and ( self:GetAttribute("state-raidmonitor") == "raid1" or self:GetAttribute("state-raidmonitor") == "raid5" ) ) then
				self:GetFrameRef("partyHeader"):Hide()
			elseif( self:GetAttribute("hideSemiRaid") and self:GetAttribute("state-raidmonitor") == "raid5" ) then
				self:GetFrameRef("partyHeader"):Hide()
			else
				self:GetFrameRef("partyHeader"):Show()
			end
		]])
		RegisterStateDriver(stateMonitor, "raidmonitor", "[target=raid5, exists] raid5; [target=raid1, exists] raid1; none")
	else
		headerFrame:Show()
	end
end
...
function Units:LoadChildUnit(parent, type, id)
	...
	local frame = CreateFrame("Button", "SUFChild" .. type .. string.match(parent:GetName(), "(%d+)"), parent, "SecureUnitButtonTemplate")
	self:CreateUnit(frame)
	...
	if( parent.unitRealType ~= "party" and parent.unitRealType ~= "raid" ) then return end
	...
	if( not parent.isWrapped ) then
		...
		stateMonitor:WrapScript(parent, "OnAttributeChanged", [[
			if( name ~= "unit" or not value or value == "player" or self:GetAttribute("lastUnit") == value ) then return end
			...
			while( true ) do
				...
				if( child:GetAttribute("parentUnit") ~= value ) then
...
function Units:InitializeFrame(config, type)
	...
	-- Party pet/target are not created until their owners are, this will only initialize them if the owners were loaded already
	elseif( type == "partypet" or type == "partytarget" ) then
		for id, unit in pairs(ShadowUF.partyUnits) do
			if( unitFrames[unit] ) then
				self:LoadChildUnit(unitFrames[unit], type, id)
			end
		end
	-- As with party pet and targets, the owners need to be made first
	elseif( type == "maintanktarget" or type == "mainassisttarget" ) then
		for _, frame in pairs(frameList) do
			if( ( type == "maintanktarget" and frame.unitType == "maintank" ) or ( type == "mainassisttarget" and frame.unitType == "mainassist" ) ) then
...
function Units:UninitializeFrame(config, type)
	...
	if( type == "raid" and loadedUnits.party ) then
...
function Units:ProfileChanged()
	...
	for _, frame in pairs(frameList) do
		if( frame.unit and loadedUnits[frame.unitType] ) then
	...
	self:ReloadHeader("maintank")
	self:ReloadHeader("mainassist")
end
