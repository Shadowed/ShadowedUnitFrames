	...
	for tag in string.gmatch(tags, "%[(.-)%]") do
		...
		if( tagEvents ) then
			for event in string.gmatch(tagEvents, "%S+") do
				...
				if( Tags.eventType[event] == "power" ) then
					fontString.fastPower = true
				end
				
				if( Tags.eventType[event] == "health" ) then
...
Tags.defaultHelp = {
	...
	["pvp:time"]			= L["Shows how long until your PVP flag drops, will not show if the flag is manually on or you are in a hostile zone."],
	...
	["color:sit"]			= L["Returns a color code of the threat situation with your target: Red for Aggro, Orange for High threat and Yellow to watch out."],
...
Tags.defaultNames = {
	...
	["abs:incheal"]			= L["Incoming heal (Absolutely)"],
...
function Tags:Verify()
	...
	for tag, data in pairs(self.defaultTags) do
		...
		if( not self.defaultCategories[tag] ) then
			print(string.format("No category found for %s.", tag))
			fine = nil
		end
	end
