...
local unitOrder, positionList, fullReload, pointPositions, isModifiersSet, isUnitDisabled, mergeTables, hideBasicOption, unitCategories
local function loadData()
	playerClass = select(2, UnitClass("player"))

	unitCategories = {
		player = {"player", "pet", "target"},
		general = {"pettarget", "targettarget", "targettargettarget", "focus", "focustarget"},
	...
	fullReload = {["bars"] = true, ["auras"] = true, ["backdrop"] = true, ["font"] = true, ["classColors"] = true, ["powerColors"] = true, ["healthColors"] = true, ["xpColors"] = true}

	quickIDMap = {}
	
	-- Helper functions
	local function getFrameName(unit)
		if( unit == "raid" or unit == "party" or unit == "maintank" or unit == "mainassist" ) then
	...
	getAnchorParents = function(info)
		...
		if( unit == "partypet" or unit == "partytarget" ) then
			...
			return anchorList
		elseif( unit == "mainassisttarget" ) then
			anchorList["$parent"] = L["Main assist member"]
			return anchorList
		elseif( unit == "maintanktarget" ) then
			anchorList["$parent"] = L["Main tank member"]
			return anchorList
		end
	...
	getName = function(info)
		...
		return L.classes[key] or L.indicators[key] or L.units[key] or L.tags[key] or L[key]
	end
	...
	local function specialRestricted(unit, moduleKey, moduleSubKey, key)
		if( string.match(unit, "%w+target") and ( key == "colorAggro" or key == "aggro" or key == "incHeal" ) ) then
			return true
...
local function loadGeneralOptions()
	...
	Config.hideTable = {
		...
		hidden = function(info)
			return info[#(info)] == "targettarget" and ShadowUF.isBuild30300
		end,
		get = get,
		arg = "hidden.$key",
	}
					...
					general = {
						...
						args = {
							...
							auraBorder = {
								...
								values = {["dark"] = L["Dark"], ["light"] = L["Light"], ["blizzard"] = L["Blizzard"]},
...
local function loadUnitOptions()
	...
	local function setPosition(info, value)
		...
		if( info[2] == "raid" or info[2] == "maintank" or info[2] == "mainassist" or info[2] == "party" ) then
		...
		args = {
			general = {
				...
				args = {
					vehicle = {
						...
						name = L["Vehicles"],
						hidden = function(info)
							if( ShadowUF.db.profile.advanced and info[2] == "party" ) then
								return false
							end
							
							return info[2] ~= "player"
						end,
						args = {
							disable = {
								order = 0,
			...
			attributes = {
				...
				hidden = function(info)
					...
					return unit ~= "raid" and unit ~= "party" and unit ~= "mainassist" and unit ~= "maintank"
				end,
				set = function(info, value)
					setUnit(info, value)
				...
				args = {
							...
							help = {
								...
								args = {
									help = {
										...
										type = "description",
										name = function(info)
											return info[2] == "party" and L["Showing the player frame in party will use the party frame configuration."] or L["Showing the party in raid frames will use all raid frame configurations, party pets and party targets are automatically disabled as well."]
										end,
									},
								},
							},
					...
					sort = {
						...
						inline = true,
						name = L["Sorting"],
						hidden = function(info) return info[2] == "raid" or not ShadowUF.db.profile.advanced end,
						args = {
							sortMethod = {
								order = 2,
								type = "select",
					...
					bar = {
						...
						args = {
							...
							sep = {
								...
								type = "description",
								name = "",
								hidden = function(info) return playerClass ~= "DRUID" and playerClass ~= "SHAMAN" and playerClass ~= "DEATHKNIGHT" end,
							},
							powerBar = {
								order = 3,
								type = "toggle",
	...
	local unitCategory = {
		order = function(info)
			local cat = info[#(info)]
			return cat == "playercat" and 50 or cat == "generalcat" and 100 or cat == "partycat" and 200 or cat == "raidcat" and 300
		end,
		type = "header",
		name = function(info)
			local cat = info[#(info)]
			return cat == "playercat" and L["Player units"] or cat == "generalcat" and L["General units"] or cat == "raidcat" and L["Raid units"] or cat == "partycat" and L["Party units"]
		end,
		width = "full",
	}
	...
	for cat, list in pairs(unitCategories) do
		...
		for _, unit in pairs(list) do
			unitCatOrder[unit] = cat == "player" and 50 or cat == "general" and 100 or cat == "party" and 200 or cat == "raid" and 300
		end
	end
...
SLASH_SUF1 = "/suf"
SLASH_SUF2 = "/shadowuf"
SLASH_SUF3 = "/shadoweduf"
SLASH_SUF4 = "/shadowedunitframes"
